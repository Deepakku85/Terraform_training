...
resource "aws_launch_template" "launch_template" {
  ...
  ty_group_names = [ module.security_group.security_group_id]
  tag_specifications {
...
resource "aws_autoscaling_group" "deep" {
    name = var.name
    launch_template {
...
resource "aws_autoscalingplans_scaling_plan" "cpu_utilization" {
  ...
  scaling_instruction {
    resource_id = format("autoScalingGroup/%s", aws_autoscaling_group.deep.name)
...
resource "aws_autoscaling_notification" "notification" {
  group_names = [ 
    aws_autoscaling_group.deep.name
  ]
